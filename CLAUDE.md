...
| `docs/ARTICLE-jobs-challenge-plan.md` | the Q16 "can the world create enough jobs?" build — the *future-quantity* companion to Q15's quality story; the 1.2bn youth wave and the demographic dividend-or-drag fork (verdict refuses to predict); AI as a featured two-beat act (exposure + the connectivity divide); 15 movements re-derived from the World Bank's *Global Jobs Challenge* (2026, CC BY 3.0 IGO) own figure workbooks via `scripts/analysis/global-jobs-challenge.ts` (transcribe-chart-pack pattern, not the registry); the bar-kit-has-no-grouped-bars → slopegraph workaround |
| `docs/ARTICLE-work-plan.md` | the Q15 "is work getting better or worse?" build — the destitution-fell-but-insecurity-didn't split; working poverty read world→income→region→country (+youth/sex) as the spine, informality as the despair counter-melody; 9 movements on the `ilostat` adapter (now multi-area) + `scripts/analysis/work-and-jobs.ts` |
| `docs/ARTICLE-airpollution-plan.md` | the Q14 "is the air cleaner or deadlier?" build — hard-balance (no lean); 8 acts / ~22 movements; signature **data-is-the-story** act (deserts→secrets→citizens→satellites) + a **forgotten-poisons** act (lead-is-cardiovascular, ammonia, indoor); the **IHME-link-only-even-through-OWID** gate trap; ACAG (CC BY) carries the re-hostable exposure load while GBD is link-only; built on a 45-beat deep-angle research pass |
